...
// Return a slice with one value.  If the integer is low
// enough we can avoid allocation by returning a slice of our
// preallocated array.
func singleSlice(value int) []int {
	if value < 1000 {
		return preallocated[value : value+1]
	}
	return []int{value}
}

// Append a value to a slice.  If the slice is a view
// onto the preallocated array then we must create a new
// slice with both values.
func appendedSlice(old []int, i int) []int {
	if len(old) == 1 && old[0] < 1000 {
		return []int{old[0], i}
	}
	return append(old, i)
}

// Overwrite a value in a slice with a new value.  If
// the slice is a view on our preallocated array then
// we return a different slice instead.
func modifySlice(old []int, index int, value int) []int {
	if len(old) == 1 && old[0] < 1000 {
		return singleSlice(value)
	}
	old[index] = value
	return old
}
...
func (o *OrderedSet) Add(element interface{}) {
	...
	if indices == nil {
		...
		o.hashToIndex[hash] = singleSlice(index)
		o.len++
		return
	}
	deleted_space := -1
	...
	o.backing = append(o.backing, element)
	if deleted_space == -1 {
		o.hashToIndex[hash] = appendedSlice(indices, index)
	} else {
		o.hashToIndex[hash] = modifySlice(indices, deleted_space, index)
	}
	o.len++
}
